...
function DocumentAddButtons(props) {
  const { filteredLanguages } = useInternationalizedArrayContext(), value = isSanityDocument(props.value) ? props.value : void 0, toast = useToast(), { onChange } = useDocumentPane(), schema = useSchema(), documentsToTranslation = getDocumentsToTranslate(value, []), getInitialValueForType = useCallback((typeName) => {
    var _a;
    if (!typeName) return;
  ...
  }, [schema]), handleDocumentButtonClick = useCallback(
    async (event) => {
...
function InternationalizedInput(props) {
  ...
  ), originalOnChange = props.inputProps.onChange, wrappedOnChange = useCallback((patches) => {
    var _a;
    const valueField = (_a = props.value) == null ? void 0 : _a.value;
    if ((valueField === void 0 || Array.isArray(valueField) && valueField.length === 0) && patches.some((patch) => patch.type === "insert" && patch.path && patch.path.length > 0 ? patch.path[0] === "value" || typeof patch.path[0] == "number" : !1)) {
  ...
  }, [props.value, originalOnChange]), inlineProps = __spreadProps$2(__spreadValues$3({}, props.inputProps), {
